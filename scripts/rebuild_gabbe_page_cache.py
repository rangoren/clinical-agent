...
from services.textbook_catalog_service import cache_gabbe_page_text_batch


def main():
    parser = argparse.ArgumentParser(description="Incrementally build Gabbe page-text cache.")
    ...
    if total_pages and start_page > total_pages:
        print("Gabbe page cache is already complete.")
        return 0

    if start_page > target_page:
        print(f"Gabbe page cache already built through page {progress.get('cached_through_page')}.")
        return 0

    while start_page <= target_page:
        payload = cache_gabbe_page_text_batch(start_page=start_page, limit=batch_size)
        print(
        ...
        if payload["end_page"] >= total_pages:
            print("Reached end of Gabbe PDF.")
            break
